...
def evaluate_test_set_hierarchical(model, loader, device, min_size):
    ...
    print(f"\nRunning Comprehensive Evaluation on 2.5D Test Set (Component filter ≥{min_size}px)...")
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc="[Evaluating Test Slices]"):
            ...
            preds_raw = (torch.sigmoid(outputs) > 0.5).float()
...
def main():
    ...
    test_dataset = LIDC25DDataset(args.manifest, split="test", transform=val_transforms)
    print(f"Loaded {len(test_dataset)} test slices from {args.manifest}.")
    ...
    print_and_format_report(results, min_size=args.min_size, report_path=args.report_path)
