...
def load_patient_dicom_and_xml(patient_id, dataset_dir="datasetmare", xml_index=None):
    """
...
def process_single_patient(
    ...
    """
    volume, slices_info, matched_xml, clean_id = load_patient_dicom_and_xml(patient_id, dataset_dir, xml_index)
...
def _patient_worker(task):
    ...
    """
    pid, dataset_dir, output_dir, split, xml_index = task
    try:
...
def main():
    ...
    tasks = [
        (os.path.basename(ppath), args.dataset_dir, args.output_dir, split_map.get(os.path.basename(ppath), "train"), xml_index)
