...
def get_model(model_type, in_channels=3, channels=None, strides=None):
    """
...
class LIDC25DDataset(Dataset):
    ...
    Target mask is the 1-channel centre-slice mask [z] (1, H, W).
    ...
    def __init__(self, manifest_csv, split="train", transform=None, neg_ratio=1.5, seed=42,
                 store_dir=None, in_slices=3, nodule_size_csv=None):
        assert in_slices % 2 == 1, "in_slices must be odd so the target slice sits in the middle"
        ...
        # slices per patient across ALL splits: the 2.5D neighbour clamp needs the true
        # volume length, not the length of this split
    ...
    def __getitem__(self, idx):
        ...
        zs = tuple(min(max(z + o, 0), last) for o in range(-self.half, self.half + 1))

        image, mask = self._read(pat_i, z, zs)
...
def get_transforms(no_augmentations=False, resize=True):
    """
    Returns MONAI transform pipelines for 2.5D inputs.
...
def train_epoch(model, loader, optimizer, loss_fn, device, epoch, total_epochs, scaler=None, amp_dtype=torch.float16, ema=None):
    """
    Trains MONAI 2.5D UNet for 1 epoch using PyTorch AMP (BF16 / FP16) precision.
    """
    model.train()
    running_loss = 0.0
    n_skipped = 0
...
def main():
    ...
    parser.add_argument("--in_slices", type=int, default=DEFAULT_IN_SLICES, help=f"Number of adjacent Z slices fed as channels; must be odd. A wider window covers more of a nodule's Z extent at proportionally more compute (default: {DEFAULT_IN_SLICES})")
    ...
    train_dataset = LIDC25DDataset(args.manifest, split="train", transform=train_transforms,
                                   neg_ratio=args.neg_ratio, seed=args.seed, store_dir=store_dir,
                                   in_slices=args.in_slices,
    ...
    model, model_kwargs = get_model(args.model_type, in_channels=args.in_slices)
    ...
    print(f"Validation gate: th={VAL_GATE['threshold']}, swept over {n_cfg} "
          ...
          f"in_slices={args.in_slices} "
          f"| ema={args.ema_decay} | warmup={args.warmup_epochs}")

    # Resume capability from checkpoint
    if args.resume and os.path.exists(checkpoint_path):
        checkpoint = torch.load(checkpoint_path, map_location=device, weights_only=False)
        ck_slices = checkpoint.get("in_slices", 3)
        if ck_slices != args.in_slices:
            raise SystemExit(f"Checkpoint was trained with --in_slices {ck_slices}, "
                             f"but this run passes {args.in_slices}. Refusing to resume.")
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        torch.save({
            ...
            "in_slices": args.in_slices, "model_type": args.model_type,
